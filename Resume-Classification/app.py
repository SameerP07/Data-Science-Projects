...
def get_top_keywords(cleaned_text: str, top_n: int = 20):
    """
    Returns top_n keywords from the resume based on TF-IDF scores.
    """
    if not cleaned_text or len(cleaned_text.strip()) < 5:
        return []

    X = tfidf.transform([cleaned_text])  # sparse row vector
    if X.nnz == 0:
        return []
    ...
    row = X.tocoo()
    pairs = list(zip(row.col, row.data))  # (feature_index, tfidf_score)
    pairs.sort(key=lambda x: x[1], reverse=True)
    ...
    for idx, score in pairs:
        ...
        if word not in seen:
            keywords.append((word, float(score)))
            seen.add(word)
...
with mid:
    ...
    if uploaded_file is not None:
        ...
        if analyze:
            with st.spinner("🔍 Extracting text and predicting..."):
                ...
                if len(cleaned.strip()) < 30:
                    ...
                    keywords = get_top_keywords(cleaned, top_n=20)
        ...
        if st.session_state.processed and st.session_state.prediction:
            ...
            # Keywords block
            st.write("")
            st.markdown("**🔑 Resume Keywords (Top TF-IDF)**")
            if st.session_state.keywords:
                chips_html = "".join(
                    [f"<span class='chip'>#{w}</span>" for (w, _s) in st.session_state.keywords]
                )
                st.markdown(chips_html, unsafe_allow_html=True)
            else:
                st.caption("No keywords found (resume text may be too short or not in TF-IDF vocab).")
            ...
            if st.session_state.keywords:
                report += "Keywords (Top TF-IDF):\n"
                report += ", ".join([w for (w, _s) in st.session_state.keywords]) + "\n\n"
